use chia_pos2::{
    Prover, create_v2_single_plot_group, plot_id_for_index, solve_proof, validate_proof_v2,
};
use clap::Parser;
...
fn main() {
    ...
    let plot_group_id: [u8; 32] = Sha256::digest(args.plot_seed.to_be_bytes()).into();

    let k = 18;
    let strength = 2;
    let index = 0;
    let meta_group = 0;
    let plot_filename = format!("k-18-test-{}.plot2", hex::encode(plot_group_id),);
    ...
    loop {
        ...
        for q in qualities {
            ...
            if !args.disable_solving {
                let plot_id = plot_id_for_index(&plot_group_id, q.plot_index, meta_group).unwrap();

                let full_proof = solve_proof(&q.chain, &plot_id, k, strength);
