...
class PartitionedProver
{
public:
    PartitionedProver(const std::array<uint8_t, 32> &challenge, const std::string &plot_file_name)
        : challenge_(challenge), plot_file_name_(plot_file_name)
    {
    }

    // initializer with challenge and plot file contents for testing
    PartitionedProver(const std::array<uint8_t, 32> &challenge, const PlotFormat &plot_format)
        : challenge_(challenge), plot_(plot_format)
    {
    }

    ~PartitionedProver() = default;

    void readHeadersFromFile()
    {
        if (!plot_.has_value()) {
            std::cout << "Prover: has no plot, initializing with " << plot_file_name_ << std::endl;
            plot_ = PlotFormat(plot_file_name_);
        }
        else {
    ...
    std::vector<QualityChain> prove(int proof_fragment_scan_filter_bits)
    {
        ...
        // Ensure we have a PlotFormat instance loaded (construct from filename on first use)
        if (!plot_.has_value()) {
        ...
        ProofCore proof_core(plot_.value().getParams());
        ...
        // 2) Scan the plot data for fragments that pass the Proof Fragment Scan Filter
        ProofFragmentScanFilter scan_filter(plot_.value().getParams(), next_challenge, proof_fragment_scan_filter_bits);
        ProofFragmentScanFilter::ScanRange range = scan_filter.getScanRangeForFilter();
        size_t challenge_partition = 0; // need function to get partition for challenge.
        plot_.value().ensurePartitionT4T5BackPointersLoaded(challenge_partition);
        std::vector<ProofFragmentScanFilter::ScanResult> filtered_fragments = scan_filter.scan(plot_.value().getContents().data.t3_proof_fragments[challenge_partition]);
        ...
        return processFilteredFragments(plot_.value().getContents(), filtered_fragments, next_challenge);
    }

    // Build quality chains from the filtered fragments
    std::vector<QualityChain> processFilteredFragments(
        const PlotFormat::PlotFormatContents &plot,
        ...
        const BlakeHash::Result256 &next_challenge)
    {
        ...
        for (const auto &frag_res : filtered_fragments)
        {
            ...
            #endif
            plot_->ensurePartitionT4T5BackPointersLoaded(l_partition);
            plot_->ensurePartitionT4T5BackPointersLoaded(r_partition);
    ...
    std::vector<QualityLink> getFirstQualityLinks(FragmentsParent /*parent*/, FragmentsPattern required_pattern, uint32_t t3_fragment_index, uint32_t t4_partition)
    {
        ...
        std::vector<ProofFragment> t3_proof_fragments = plot_.value().getContents().data.t3_proof_fragments[t4_partition];
        std::vector<PartitionedBackPointer> t4_to_t3_back_pointers = plot_.value().getContents().data.t4_to_t3_back_pointers[t4_partition];
        std::vector<T5PlotBackPointers> t5_to_t4_back_pointers = plot_.value().getContents().data.t5_to_t4_back_pointers[t4_partition];
