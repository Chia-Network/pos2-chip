...
int main(int argc, char *argv[])
{
    ...
    // get t5 pairing
    int partition = 0;
    T5Pairing t5_pairing = plot.data.t5_to_t4_back_pointers[partition][0];    // now get t4 L and R pairings
    ...
    for (int i=0;i<8; i++)
    {
        ...
        if (i % 2 == 0) {
            x_values.push_back(plot.data.xs_correlating_to_encrypted_xs[t4_to_t3_L.encx_index_l][i]);
        }
    }
    for (int i=0;i<8; i++)
    {
        ...
        if (i % 2 == 0) {
            x_values.push_back(plot.data.xs_correlating_to_encrypted_xs[t4_to_t3_L.encx_index_r][i]);
        }
    }
    for (int i=0;i<8; i++)
    {
        ...
        if (i % 2 == 0) {
            x_values.push_back(plot.data.xs_correlating_to_encrypted_xs[t4_to_t3_R.encx_index_l][i]);
        }
    }
    for (int i=0;i<8; i++)
    {
        ...
        if (i % 2 == 0) {
            x_values.push_back(plot.data.xs_correlating_to_encrypted_xs[t4_to_t3_R.encx_index_r][i]);
        }
    }
    std::cout << std::endl;

    Solver solver(plot.params);
    solver.solve(x_values, xs_solution);
}