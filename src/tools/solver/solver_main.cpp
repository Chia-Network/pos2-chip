...
int benchmark(const uint8_t k, const uint8_t plot_strength)
{
...
int xbits(const std::string &plot_id_hex, const std::vector<uint32_t> &x_bits_list, const uint8_t k, const uint8_t strength)
{
...
try
{
    ...
    if (mode == "benchmark")
    {
        ...
        return benchmark(static_cast<uint8_t>(k), static_cast<uint8_t>(plot_strength));
    }
    else if (mode == "xbits")
    {
        ...
        const size_t xbits_hex_len = xbits_hex.length();
        std::vector<uint32_t> x_bits_list;
        const size_t calculated_k = xbits_hex_len / 32; // each uint32_t is 4 hex characters
        ...
        x_bits_list = Utils::compressedHexToKValues(static_cast<int>(calculated_k / 2), xbits_hex);
        ...
        return xbits(plot_id_hex, x_bits_list, static_cast<uint8_t>(calculated_k), static_cast<uint8_t>(plot_strength));
    }
    else
    {
